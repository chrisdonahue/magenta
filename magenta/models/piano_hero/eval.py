...
def main(unused_argv):
  ...
  if cfg.stp_emb_vq or cfg.stp_emb_iq:
    ...
    weighted_avg = lambda t, m: tf.reduce_sum(tf.multiply(t, m)) / tf.reduce_sum(m)
  ...
  for name, tensor in summary_name_to_batch_tensor.items():
  ...
  def _eval_all(sess):
    ...
    if gold:
      for _midi_notes, _buttons, _seq_varlen in gold_iterator([-6, 6]):
        _gold_diff_l1, _gold_diff_l2 = sess.run(
            [gold_diff_l1, gold_diff_l2], {
                gold_feat_dict["midi_pitches"]:
                    _midi_notes,
                gold_feat_dict["delta_times_int"]:
                    np.ones_like(_midi_notes) * 8,
                gold_seq_varlens: [_seq_varlen],
                gold_buttons:
                    _buttons
            })
        _summaries["gold_diff_l1"].append(_gold_diff_l1)
        _summaries["gold_diff_l2"].append(_gold_diff_l2)

    while True:
      try:
        _batches = sess.run(summary_name_to_batch_tensor)
      except:
        break

      for name, _scalar in _batches.items():
        _summaries[name].append(_scalar)
    ...
    while True:
      ...
      if latest_ckpt_fp != ckpt_fp:
        ...
        with tf.Session() as sess:
          ...
          _summaries, _step = sess.run(
              [summaries, step],
              feed_dict={
                  summary_name_to_placeholder[n]: v
                  for n, v in _summaries.items()
              })
