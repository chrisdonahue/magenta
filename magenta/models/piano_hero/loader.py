...
def load_noteseqs(fp,
                  ...
                  repeat=False,
                  buffer_size=512):
  """Loads random subsequences from NoteSequences in SSTables.
  ...
  def _str_to_tensor(note_sequence_str,
    ...
    note_sequence_ordered = filter(
        lambda n: (n.pitch >= 21) and (n.pitch <= 108), note_sequence_ordered)
    ...
    if len(note_sequence_ordered) > 0:
  ...
  dataset = dataset.map(_example_to_serialized_note_sequence)

  # Deserialize protos
  dataset = dataset.map(
      lambda data: tf.py_func(
          lambda x: _str_to_tensor(x, augment_stretch_bounds, augment_transpose_bounds),
